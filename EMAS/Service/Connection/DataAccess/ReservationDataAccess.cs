...
namespace EMAS.Service.Connection.DataAccess
{
    public class ReservationDataAccess : IObjectStateLocationBoundedDataAccess<Reservation>
    {
        ...
        public List<Reservation> SelectOnLocation(int locationId)
        {
            ...
            string query = "SELECT E.date, R.start_event_id, R.additional_info, E.employee_id" +
            ...
            while (reader.Read())
            {
                reservations.Add(new Reservation(reader.GetInt64(1), reader.GetDateTime(0), employeeAccess.SelectById(reader.GetInt32(3)), reader.GetString(2), storableObjectInEventDataAccess.SelectObjectsInEvent(reader.GetInt64(1))));
        ...
        public void Complete(Reservation[] reservations)
        {
            foreach (var completedReservation in reservations)
            {
                ...
                using var command = new NpgsqlCommand("UPDATE \"event\".reservation SET end_event_id=@end_event_id, WHERE start_event_id=@start_event_id ", connection);
